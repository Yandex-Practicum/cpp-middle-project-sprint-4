...
std::vector<Function> FunctionExtractor::Get(const analyser::file::File& file) {
    ...
    while ((start = ast.find(marker, start)) != std::string::npos) {
        ...
        Function func{
            .filename = file.name, .class_name = std::nullopt, .name = func_name, .ast = func_ast};
...
FunctionExtractor::FunctionNameLocation FunctionExtractor::GetNameLocation(
    const std::string& function_ast) {
    ...
    Position start{static_cast<size_t>(std::stoi(coords.substr(0, comma))),
                   static_cast<size_t>(std::stoi(coords.substr(comma + 1)))};
    ...
    Position end{static_cast<size_t>(std::stoi(end_coords.substr(0, comma))),
                 static_cast<size_t>(std::stoi(end_coords.substr(comma + 1)))};
...
std::optional<FunctionExtractor::ClassInfo> FunctionExtractor::FindEnclosingClass(
    const std::string& ast, const FunctionNameLocation& func_loc) {
    size_t class_pos = 0;
    ...
    while ((class_pos = ast.find(class_marker, class_pos)) != std::string::npos) {
        ...
        size_t comma = coords.find(',');
        Position class_start{static_cast<size_t>(std::stoi(coords.substr(0, comma))),
                             static_cast<size_t>(std::stoi(coords.substr(comma + 1)))};

        size_t dash = ast.find('-', coord_end);
        size_t second_coord_start = ast.find('[', dash);
        size_t second_coord_end = ast.find(']', second_coord_start);
        std::string end_coords =
            ast.substr(second_coord_start + 1, second_coord_end - second_coord_start - 1);

        comma = end_coords.find(',');
        Position class_end{static_cast<size_t>(std::stoi(end_coords.substr(0, comma))),
                           static_cast<size_t>(std::stoi(end_coords.substr(comma + 1)))};
            ...
            if (func_loc.start.line < class_end.line ||
                ...
                if (name_start != std::string::npos) {
                    ...
                    if (id_start != std::string::npos) {
                        ...
                        std::string id_coords =
                            ast.substr(id_coord_start + 1, id_coord_end - id_coord_start - 1);
